...
const runtimeConfig = (() => {
  try {
    return JSON.parse(process.env.CLOUD_RUNTIME_CONFIG || "{}") as Record<string, any>;
  } catch {
    return {};
  }
})();

const refundConfig = (runtimeConfig.refund || {}) as Record<string, unknown>;
const emailConfig = (runtimeConfig.email || {}) as Record<string, unknown>;

const resolveRefundConfigValue = (envKey: string, configKey: string, fallback = "") => {
...
const REFUND_FROM_EMAIL = resolveRefundConfigValue(
  "REFUND_SMTP_FROM",
  "smtp_from",
...
const SMTP_SECURE =
  resolveRefundConfigValue("REFUND_SMTP_SECURE", "smtp_secure", "").toLowerCase() === "true" ||
  SMTP_PORT === 465;
...
const requireMaster = async (req: any, res: any) => {
...
type PlanType = "annual" | "monthly" | "test" | "unknown";
type SaleSourceType = "checkout_payment" | "invoice_payment" | "grant_entitlement";

interface SaleSyncInput {
...
const planLabel = (planType: PlanType) => {
  ...
  if (planType === "monthly") return "Plano mensal";
  if (planType === "test") return "Teste 7 dias";
  return "Venda";
};
...
const resolveSalesDestination = async () => {
  ...
  const cryptoEpoch =
    typeof ownerData.cryptoEpoch === "number" && Number.isFinite(ownerData.cryptoEpoch)
...
const resolveChargeFinancials = async (
  stripe: Stripe,
  chargeId: string
): Promise<ChargeFinancials | null> => {
  ...
  const balanceRaw = charge.balance_transaction as unknown;
  if (balanceRaw && typeof balanceRaw === "object") {
    const maybeFee = (balanceRaw as { fee?: unknown }).fee;
    const maybeNet = (balanceRaw as { net?: unknown }).net;
    ...
    try {
      ...
      if (typeof balanceTx.fee === "number") feeCents = balanceTx.fee;
      if (typeof balanceTx.net === "number") netCents = balanceTx.net;
      else netCents = grossCents - feeCents;
    } catch (error) {
      console.error("[sales] balance_transaction_error", error);
    }
  }

  if (!Number.isFinite(netCents)) netCents = grossCents - feeCents;
...
const resolveChargeIdFromPaymentIntent = async (stripe: Stripe, paymentIntentId: string) => {
  if (!paymentIntentId) return "";
  const intent = await stripe.paymentIntents.retrieve(paymentIntentId, { expand: ["latest_charge"] });
  return extractExpandableId(intent.latest_charge);
};

const upsertStripeSaleIncome = async (
  stripe: Stripe,
  input: SaleSyncInput
) => {
  ...
  const ledgerId = toSafeDocId(
    paymentIntentId ? `pi_${paymentIntentId}` : `src_${input.sourceType}_${sourceId}`
  );
...
const markSalesIncomeAsRefunded = async (params: {
  ...
  const loadByField = async (field: string, value: string) => {
    if (!value) return;
  ...
  for (const doc of matches.values()) {
    ...
    const ownerId =
      typeof data.ownerId === "string" && data.ownerId.trim() ? data.ownerId.trim() : SALES_OWNER_ID;
...
const resolvePlanFromCheckoutSession = (
  session: Stripe.Checkout.Session
): PlanType => {
  const metadataPlan =
    session.metadata && typeof session.metadata.plan === "string"
      ? normalizePlanType(session.metadata.plan)
      : "unknown";
  if (metadataPlan !== "unknown") return metadataPlan;
  if (session.mode === "subscription") return "monthly";
  if (session.mode === "payment") return "annual";
  return "unknown";
};

export const stripeWebhookV2 = onRequest(
  { secrets: [STRIPE_SECRET_KEY_SECRET, STRIPE_WEBHOOK_SECRET] },
  async (req, res) => {
...
export const createCheckoutSessionV2 = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const planType =
        plan === "monthly" ? "monthly" : plan === "test" ? "test" : "annual";
      const priceId =
        planType === "monthly"
          ? monthlyPriceId
          : planType === "test"
...
export const requestRefundV2 = onRequest(
  { secrets: [STRIPE_SECRET_KEY_SECRET] },
  async (req, res) => {
    applyCors(req.headers.origin as string | undefined, res);
    if (req.method === "OPTIONS") {
    ...
    const refundTimelineNotice =
      "Normalmente o estorno aparece em 5–10 dias úteis, dependendo do banco. Em alguns casos ele aparece como reversão da cobrança (a compra original some) e não como crédito separado. Se passar de 10 dias úteis, fale com o banco.";
    const adminText = [
      `Nome: ${name}`,
      `E-mail: ${email}`,
    ...
    if (entitlementData.status && entitlementData.status !== "active") {
      await refundRef.set(
        { status: "already_processed", updatedAt: admin.firestore.FieldValue.serverTimestamp() },
        { merge: true }
      );
      res.status(200).json({
        ok: true,
    ...
    const finalizeEntitlement = async (updates: Record<string, unknown>) => {
    ...
    const finalizeRefund = async (updates: Record<string, unknown>) => {
      await refundRef.set(
        {
          ...updates,
          updatedAt: admin.firestore.FieldValue.serverTimestamp()
    ...
    const attemptRefundWithIntent = async () => {
      ...
      try {
        ...
        await sendRefundEmail(
          ...
          adminText,
          "Reembolso aprovado",
          `Seu reembolso foi aprovado e já foi solicitado ao meio de pagamento.\n${refundTimelineNotice}`
        );
        res.status(200).json({
          ok: true,
          status: "refunded",
...
export const masterMetrics = onRequest(
  {
  ...
  async (req, res) => {
    ...
    try {
      ...
      const monthLabelFromYearMonth = (year: number, monthIndex: number) =>
        new Date(Date.UTC(year, monthIndex, 1))
          .toLocaleDateString("pt-BR", { month: "short", year: "2-digit", timeZone: "UTC" })
          .replace(".", "");
      const yearKeyFromMs = (ms: number) => String(new Date(ms).getUTCFullYear());
      const countBefore = (values: number[], thresholdMs: number) =>
        values.reduce((sum, item) => sum + (item < thresholdMs ? 1 : 0), 0);
      ...
      const isExpired = (data: any) => {
        ...
        if (!raw) return false;
        if (typeof raw.toMillis === "function") return raw.toMillis() <= now;
        if (typeof raw.seconds === "number") return raw.seconds * 1000 <= now;
        return false;
      };

      const entitlementsActive = entitlements.filter(
        (ent) => ent?.status === "active" && !isExpired(ent)
      ).length;
      const entitlementsExpired = entitlements.filter((ent) => isExpired(ent)).length;

      const stripeEntitlements = entitlements.filter((ent) =>
        String(ent?.source || "").startsWith("stripe")
      );
      ...
      let revenueEstimateCents: number | null = null;
